...
Lets look at fairy shrimp species from the UC Merced vernal pools reserve [https://vernalpools.ucmerced.edu/](https://vernalpools.ucmerced.edu/)
...
This is the list of tips in our tree
```{r}
sort(shrimp_tr$tip.label)
```

And this is the list of species we have location data for
```{r}
sort(unique(shrimp_locs[,1]))
```


## We don't have location data for some of our species! 
(unless somone has added records to gbif since I wrote this tutorial :P)
We can prune the species we don't have information for out of our tree, and map the tips that are left
